...
//    uint32_t ticks = (SYSCLK / MICROSECONDS_PER_SECOND) * delay; // Calculate the number of ticks for the specified delay in microseconds
...
void SysTick_Delay_us(uint32_t delay){
    while(!READ_REG_FIELD(SysTick->CTRL,SysTick_CTRL_COUNTFLAG_Msk));
}
